...
SELECT b.booking_id, b.guest_id, b.room_id, b.check_in, b.check_out
	FROM Bookings b
	WHERE YEAR(b.check_in) = 2024;

-- Cho biết số lượng phòng của từng loại
SELECT r.room_type, COUNT(*) AS total_rooms
	FROM rooms r
...
SELECT b.room_id, (SELECT r.room_type
					FROM Rooms r
	...
	HAVING COUNT(b.booking_id) = (
		SELECT MAX(cnt)
		FROM (
			SELECT COUNT(b.booking_id) AS cnt
			FROM Bookings b
			GROUP BY room_id
		) AS temp
	);
