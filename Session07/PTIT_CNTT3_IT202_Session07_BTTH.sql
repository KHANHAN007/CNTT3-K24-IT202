...
SELECT customer_id
FROM DON_HANG d
JOIN CHI_TIET_DON_HANG ctdh ON d.order_id = ctdh.order_id
JOIN SAN_PHAM sp ON ctdh.product_id = sp.product_id
GROUP BY customer_id
HAVING SUM(ctdh.quantity * sp.price) > (
    SELECT AVG(total_spent)
    FROM (
        SELECT SUM(ctdh.quantity * sp.price) AS total_spent
        FROM DON_HANG d
        JOIN CHI_TIET_DON_HANG ctdh ON d.order_id = ctdh.order_id
        JOIN SAN_PHAM sp ON ctdh.product_id = sp.product_id
        GROUP BY customer_id
...
WHERE NOT EXISTS (
    ...
    JOIN SAN_PHAM sp ON ctdh.product_id = sp.product_id
    WHERE ctdh.order_id = d.order_id
      AND sp.price <= (SELECT AVG(price) FROM SAN_PHAM)
);
