...
-- bai 1 
DELIMITER //
CREATE PROCEDURE get_post_by_user_id(
	IN p_user_id INT
)
begin 
	select
...
CALL get_post_by_user_id(1);

DROP PROCEDURE IF EXISTS get_post_by_user_id;
...
CALL CalculatePostLikes(101, @my_likes);
SELECT @my_likes ;
...
CALL CalculateBonusPoints(1, @diem_thuong);
...
CALL CreatePostWithValidation(1, 'Alo', @thong_bao);
SELECT @thong_bao AS 'Kết quả Test 1 (Ngắn)';

CALL CreatePostWithValidation(1, 'helloemyeu', @thong_bao);
SELECT @thong_bao ;
...
begin
    set activity_score = 
        (select count(*) from posts where user_id = p_user_id) * 10 +
        (select count(*) from comments where user_id = p_user_id) * 5 +
        (select count(*) from likes l join posts p on l.post_id = p.post_id where p.user_id = p_user_id) * 3;
...
call CalculateUserActivityScore (1, @diem, @xeploai);

select @diem as 'điểm số', 
		@xeploai as 'xếp loại';
